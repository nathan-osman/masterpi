PKG = github.com/nathan-osman/masterpi
CMD = masterpi
...
dist/${CMD}: ${SOURCES} ab0x.go | cache/lib cache/src/${PKG} dist
	@docker run \
	    --rm \
	    -e GOOS=linux \
	    -e GOARCH=arm \
...
ab0x.go: ${RESOURCES} dist/fileb0x b0x.yaml
